<# 
  EZ Fed Mine — One‑Click Monero P2Pool Launcher (v4b)
  - Avoids ARM64 assets; prefers windows-x64, then windows-gcc-x64
  - Robust GitHub asset discovery + retries
  - Console persists; ESC to close after errors or after miner exits
  - Logs clear Defender/AV guidance if xmrig.exe is blocked
#>
...
function Get-XmrigAssetUrl {
  ...
  if (-not $asset) {
    throw "Could not find a Windows x64 asset in latest release (non-ARM)."
  }
...
function Ensure-Xmrig {
  ...
  try {
    ...
    # Last-resort pinned pattern: generic latest/download non-ARM name (may change in future)
