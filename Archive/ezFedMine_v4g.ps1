<# 
  EZ Fed Mine — One‑Click Monero P2Pool Launcher (v4g)
  - Fix: PowerShell string parsing for "$Var:$OtherVar" using "{0}:{1}" -f ...
  - DNS self-heal (Admin) + resilient pool order (LAN → DuckDNS → WAN)
  - Robust XMRig download (Windows x64 only) with retries
  - Keeps console open; ESC to exit after errors or miner exit
#>
...
function Build-PoolList {
  $lan    = ("{0}:{1}" -f $LanIPv4, $Port)
  $duck   = ("{0}:{1}" -f $DuckDomain, $Port)
  $wan    = ("{0}:{1}" -f $FallbackIPv4, $Port)
  $list   = @()
...
# ---- MAIN ----
try {
  Log "Starting EZ Fed Mine (v4g)"
  Log ("Base  : {0}" -f $BaseDir)
  Log ("Pool  : {0} (LAN/DNS/WAN fallbacks)" -f ("{0}:{1}" -f $DuckDomain, $Port))
